...
export default function ReportPage() {
  ...
  const handleVodToggle = (checked: boolean) => {
    if (isAdminStopped && checked) {
      broadcast.status = "VOD"
      broadcast.vodVisibility = "PUBLIC"
      setVodVisibility(true)
      toast({
        title: "VOD 공개 처리됨",
        description: "송출 중지된 방송이 정상 VOD로 전환되었습니다.",
      })
      return
    }
  ...
  const endTime = new Date(broadcast.startAt.getTime() + 2 * 60 * 60 * 1000)
      ...
      <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 space-y-3">
              ...
              <div className="space-y-2 text-sm">
                ...
                {broadcast.status === "STOPED" && broadcast.stopReason && (
                  <div className="flex gap-2">
                    <span className="font-semibold w-32 text-muted-foreground">사유:</span>
                    <span className="text-red-600">{broadcast.stopReason}</span>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-5 gap-4">
          ...
          <Card className="p-4">
            ...
            <div className="text-2xl font-bold text-red-500">{broadcast.reportCount}</div>
          </Card>
